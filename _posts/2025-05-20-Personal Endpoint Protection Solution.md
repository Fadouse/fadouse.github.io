...
在 2026 年，我购买渠道 mssp 的 deepinstinct 终于更新了 对 windows 25h2/24h2 的支持。最关键的是 DI 的内存(主防)防御居然和 S1 不冲突了。 所以我又回归到了 DI + S1。
这两套目前应该是 Windows 下最佳配置了

我的配置非常简单，DI拉满！SI 关静态扫描

DI 的 静态AI检出 是当之无愧的 t0, 且占用极低
DI 的 动态检出也还行，各种单步检测和shellcode检测效果可以说也算是t1
S1 的 动态AI也说的过去，基于上下文的AI behavior打分
S1 的 EDR 可用性，易用性 T0，事件细腻程度可能比不上PA和CS 但也还是 T0级别的，配合star rules可实现云端匹配指标TTP并自动化相应
S1 最厉害的就是其回滚能力，可以针对任何storyline(基于edr关联的rca id)，仅回滚对应storyline的影响，不用重启，不影响其他软件运行
*额外优点，这两的ui都挺好看的()


占用呢？windows 11 25h2, 开启vbs
常态CPU < 0.1%
总内存 550MB 上下

兼容性那？由于本机开启vbs和hyperv功能，部分反作弊我无法测试( 5E, ACE 等需要接管cpu虚拟化的反作弊)
兼容我电脑上反作弊
EAAC（EA的反作弊，代表作有 BF 战地系列）
VAC（CS2）
PAC（完美反作弊，*不要求接管cpu虚拟化的情况下）

唯一的小缺点还是由于 DI 的设计较为激进，会替换系统dll，导致部分应用程序无法正常加载或者行为异常，需要添加排除
除此之外几乎没有任何缺点了。
